...
class SuperJob(Parser):
	...
	def get_page_content(self, response):
		'''Поиск всех данных на одной конкретной page'''
		soup = BeautifulSoup(response.text, "html.parser")
		vacancies = soup.find_all("div", class_="_1wWd1 _3irGY")
		if len(vacancies) == 0:
			return None
		else:
			content = []
				...
				content.append(
					{
						...
						"salary" : salary_block.find("span", class_=["_3xCPT", "rygxv", "_17lam", "_3GtUQ"]).get_text().replace("&nbsp;", " ").replace("\u20bd", "руб") if ( salary_block.find("span", class_=["_3xCPT", "rygxv", "_17lam", "_3GtUQ"]).get_text().replace("&nbsp;", " ").replace("\u20bd", "руб") != None ) else "Не указано",
